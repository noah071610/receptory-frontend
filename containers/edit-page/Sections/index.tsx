...
export default function SectionLayout({
  ...
  const { selectedSection, setSelectedSection, deleteSection } = useEditStore()
  const onClickSection = (e: any) => {
    let className = e.target.getAttribute("class")
    if (!className) {
      const parentElement = e.target.parentNode
      if (parentElement) {
        className = parentElement.getAttribute("class")
        if (!className) {
          const _parentElement = parentElement.parentNode
          if (_parentElement) {
            className = _parentElement.getAttribute("class")
          }
        }
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className={cx(style.tools, { [style.active]: selectedSection?.id === section.id })}>
